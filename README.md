...
* Github `SAS` code repositories for statistical data handling: Hu's [Programming `SAS`](https://github.com/Jiangtang/Programming-SAS) and [`SAS` ListProcessing](https://github.com/Jiangtang/SAS_ListProcessing), as well as Bass's [`SAS` macros](https://github.com/scottbass/SAS/tree/master/Macro).
...
* A useful [library of `SAS` macros](https://github.com/yoba7/SAS-macros), including some tools to export a `SAS` dataset to `Sqlite`.
