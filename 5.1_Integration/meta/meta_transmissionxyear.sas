/** 
## META_TRANSMISSIONxYEAR {#meta_transmissionxyear}
...
%macro meta_transmissionxyear(cds_transxyear=, cfg=, clib=);
	...
	%if %macro_isblank(cds_transxyear) %then %do;
		...
		%else											%let cds_transxyear=META_TRANSMISSIONxYEAR;
	%end;
	...
	%if %macro_isblank(cfg) %then %do;
		%if %symexist(G_PING_INTEGRATION) %then 		%let cfg=&G_PING_INTEGRATION/meta;
		%else											%let cfg=&G_PING_ROOTPATH/5.1_Integration/meta;
	%end;
...
%mend meta_transmissionxyear;


%macro _example_meta_transmissionxyear;
	...
	%put;
%mend _example_meta_transmissionxyear;

/* Uncomment for quick testing
options NOSOURCE MRECALL MLOGIC MPRINT NOTES;
%_example_meta_transmissionxyear;
*/
