...
%macro ds_sort(idsn		/* Input reference dataset 									(REQ) */
			...
			, dupdsn=	/* Name of the DUPOUT dataset used by PROC SORT 			(OPT) */
	...
	/* ASC/DESC: check variables used for sorting */

	/* first ascending variables through ASC */
	...
	* below; not very elegant, but that's it... */
	...
	%if not %macro_isblank(dupdsn) %then	%let _proc=&_proc.%str( NODUPKEY DUPOUT=&dupdsn);
...
%macro _example_ds_sort;
	...
	%ds_sort(_dstest35, dupdsn=&dsn, asc=&asc, desc=&desc);
	%ds_print(_dstest35);
	%ds_print(&dsn);
