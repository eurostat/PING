...
%macro _example_quantile_attribute;
	...
	%end;
	%put (vi) Example with input...100;
	%quantile_attribute(100, &cond, _name_=name, _label_=label, _code_=code, _case_=case);
	%if &code^=P %then 	%put ERROR: TEST FAILED - Wrong output code &code returned for input quantile "100";
	%else %do;
		%put        name=&name;
		%put  		label=&label;
		%put  		case=&case;
