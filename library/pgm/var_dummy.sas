...
%macro var_dummy(idsn    	/* Name of input dataset                  				(REQ) */
    			, var       /* Variable(s) to be dummied              				(REQ) */
  				, odsn=     /* Name of output dataset                 				(OPT) */
   				, base=  	/* Name of the base category                          	(OPT) */
   				, prefix = 	/* Prefix for dummy variable names        				(OPT) */
   				, format =  /* format used to categorize variable     				(OPT) */
   				, name  =   /* Flag set for dummy variables' names    				(OPT) */
   				, fullrank= /* Boolean flag set for eliminating baseline category	(OPT) */
	...
	%if %macro_isblank(name) %then 		%let name=VAL;

	/* PREFIX: set the default prefix */
	%if %macro_isblank(prefix) %then 	%let prefix=D_;

	/* BASE: set the default base */
	%if %macro_isblank(base) %then 		%let base=_LAST_;

	/* FULLRANK: set default/update parameter */
	%if %macro_isblank(fullrank)  %then 	%let fullrank=YES; 
	%else									%let fullrank=%upcase(&fullrank);
 
	%if %error_handle(ErrorInputParameter, 
			%par_check(&fullrank, type=CHAR, set=YES NO) NE 0,	
			txt=!!! Parameter FULLRANK is boolean flag with values in (yes/no) !!!) %then
		%goto exit;
