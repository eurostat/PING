...
%macro list_intersection(list1, list2	/* Lists of blank-separated items 					(REQ) */
						...
						, unique=		/* Boolean flag set for unique elements 			(OPT) */
	...
	/* UNIQUE: check/set */
	%if %macro_isblank(unique)  %then 	%let unique=YES; 
	%else								%let unique=%upcase(&unique); 
	%if %error_handle(ErrorInputParameter, 
			%par_check(&unique, type=CHAR, set=YES NO) NE 0,	
			txt=!!! Parameter UNIQUE is boolean flag with values in (yes/no) !!!) %then
		%goto exit;
	...
	/* LIST1/LSIT2: deal with simple cases */
	...
	%if "&casense"="NO" %then 			%let _list2=%upcase(&list2);
	...
	%do _i=1 %to %list_length(&list1, sep=&sep);
		...
		%if &sep= %then %let _pos=%sysfunc(findw(&_list2, &_uitem));
		%else 			%let _pos=%sysfunc(findw(&_list2, &_uitem, &sep));
		%if &_pos>0 %then %do;
			%if &_isec= %then 
				/* initialise _isec */	
				%let _isec=&_item;
			%else %do;
				...
				%end;
				%else %do;
					%let _posi=%sysfunc(findw(&_isec, &_item, &sep));
					%let _posj=%sysfunc(findw(&_isec, &_uitem, &sep));
				%end;
				%if &_posi<=0 and ("&unique"="NO" or &_posj<=0) %then
	...
	%if "&unique"="NO" and "&casense"="NO" and &_nloop<2 %then %do;
