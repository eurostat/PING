...
%macro ds_check(dsn 		/* Input dataset 					(REQ) */
	...
	%else %do;
		%if %error_handle(ErrorInputLibrary, 
				%macro_isblank(lib) NE 0, mac=&_mac,		
				txt=!!! Incompatible LIB and DSN parameters !!!) %then 
			%goto exit;
		%let lib = %substr(&dsn, 1, %index(&dsn,.)-1);
		%let dsn = %substr(&dsn, %index(&dsn,.)+1) ;
	%end;
	...
	%do _i=1 %to %list_length(&dsn, sep=&SEP);
