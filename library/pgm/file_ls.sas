...
%macro file_ls(dir		/* Full path of input directory 											(REQ) */
			...
			, beg=		/* Boolean flag set to match pattern at the beginning of the file/dirname 	(OPT) */
			, end=		/* Boolean flag set to match pattern at the end of the file/dirname 		(OPT) */
			, casense=	/* Boolean flag set for case sensitive matching 							(OPT) */
			, excldir=	/* Boolean flag set for subdirectories exclusion from search				(OPT) */
			, sep=		/* Output list separator 													(OPT) */
			);
	%local _mac;
	...
	%do n = 1 %to &_memcount;
		...
		%if  %macro_isblank(_base) %then 	%let _base = &_memb;
		%else 								%let _base = &_base.&sep.&_memb;
		%next:
	%end;
...
%macro _example_file_ls;
	...
	%let casense=NO;
	%let dir=&G_PING_ESTIMATION/meta; /* G_PING_LIBCONFIG */
	...
	%let ores=&pattern..xlsx &pattern.2.xlsx;
