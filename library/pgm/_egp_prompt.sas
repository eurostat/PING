...
%macro _egp_prompt(prompt_name=	/* Name of the prompt 									(OPT) */
	...
	%else %do;
		...
		%if %upcase("&ptype")="STRING" %then 			%let ptype=CHAR; 	/* legacy... */
		%else %if %upcase("&ptype")="FLOAT" %then 		%let ptype=NUMERIC;
		%if %error_handle(ErrorInputParameter, 
				%par_check(%upcase(&ptype), type=CHAR, set=CHAR NUMERIC DATE INTEGER) NE 0, mac=&_mac,		
	...
	%else %do;
		...
		%if %error_handle(ErrorInputParameter, 
				%par_check(%upcase(&penclose), type=CHAR, set=YES NO) NE 0, mac=&_mac,		
		...
		%if &&&prompt_name._count. > 0 %then %do;
			...
			%if /* if prompt is not string then it is a numeric format */
				%upcase("&ptype")^="CHAR" 
					...
					* in the numeric method */
					%upcase("&penclose")="NO" %then
				%let go_parm = 0;
			%if %upcase("&pmultipl")^="YES" %then 
			...
			%if &go_parm < 2 %then %do;
				...
				*/
				...
				%if %upcase("&verb")="YES" %then 	%put &prompt_name.: &&&prompt_name. ;
			%end;

			%else %do;
				/* case of no multiple values */
				%if %upcase("&verb")="YES" %then 	
					%put Prompt &prompt_name. is trimmed;
				%let &prompt_name = %sysfunc(trim(&&&prompt_name));
    ...
    %end;
	%else %if %upcase("&pmultipl")="YES" %then %do;
		/* case of several program with this macro */
		%if &verb=yes %then 	
			%put Prompt &prompt_name. is not created or re-created (no need to).;
		%goto exit;
	%end;                                                                               
    %else %do;
		...
		%if %upcase("&verb")="YES" %then 	
			%put Prompt &prompt_name. is trimmed;
		/*%let &prompt_name = %cmpres(%trim(&&&prompt_name));/* this does not solve exactly the problem*/
		%let &prompt_name = %sysfunc(trim(&&&prompt_name));
