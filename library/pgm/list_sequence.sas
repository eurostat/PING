...
%macro list_sequence(len=		/* Lenght of the output generated list 									(OPT) */
	...
	%if %error_handle(ErrorInputParameter, 
			...
			txt=%bquote(!!! Parameters START, END and STEP need all to be set when LEN is not !!!)) %then
		%goto exit;
	...
	%if &isbl_len NE 1 %then %do;
		%if %error_handle(ErrorInputParameter, 
				...
				txt=%bquote(!!! Wrong input %upcase(&len) value: must be INTEGER >0 !!!)) %then
			%goto exit;
	%end;
	...
	%if &isbl_start NE 1 %then %do;
		%if %error_handle(ErrorInputParameter, 
				...
				txt=%bquote(!!! Wrong input %upcase(&start) value: must be NUMERIC !!!)) %then
			%goto exit;
	%end;
	...
	%if &isbl_end NE 1 %then %do;
		%if %error_handle(ErrorInputParameter, 
				...
				txt=%bquote(!!! Wrong input %upcase(&end) value: must be NUMERIC !!!)) %then
			%goto exit;
	%end;
	...
	%if &isbl_step NE 1 %then %do;
		%if %error_handle(ErrorInputParameter, 
				...
				txt=%bquote(!!! Wrong input %upcase(&step) value: must be NUMERIC non null !!!)) %then
			%goto exit;
	%end;
	...
	%if &isbl_step NE 1 and &isbl_start NE 1 and &isbl_end NE 1 %then %do;
		%if %error_handle(ErrorInputParameter, 
				...
				txt=%bquote(!!! Wrong setting: START must be > END when STEP < 0 !!!)) %then
			%goto exit;
	%end;
