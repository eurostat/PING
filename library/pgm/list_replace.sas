...
%macro list_replace(list 		/* List of blank separated items 					(REQ) */
	...
	/* OLD/NEW: check parameters */
	%if %error_handle(ErrorInputParameter, 
			%macro_isblank(old) or %macro_isblank(new), mac=&_mac,
			txt=%bquote(!!! Parameters OLD and NEW must be passed !!!)) %then
		%goto exit;
	...
	/* check whether it was set to NO */
	...
	%do _i=1 %to &_len;
		...
		%if &sep= %then %let _pos=%sysfunc(findw(&old, &_item));
		%else 			%let _pos=%sysfunc(findw(&old, &_item, &sep));
		%if &_pos <= 0 %then	
			%goto break;
		%do _k=1 %to &_lold;
			%let _oldit = %scan(&old, &_k, &sep);
