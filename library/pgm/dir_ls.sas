...
%macro dir_ls(dir
	...
	%if %macro_isblank(recur) %then 	%let recur=NO;
	%else 								%let recur=%upcase(recur);
	...
	/* %let _fdir=/ec/prod/server/sas/0eusilc/&dsn._dir.txt; */
	...
	*/
	...
	/* store the output (basename+paths+last updates) in the dataset */
   	data &lib..&dsn (keep=base path last);
      	array dum{7} $;
      	array dat{3} $;
      	length path base $500
        %if "&recur"="YES" %then %do;
        	temp_path temp_file $500
       	%end;
        	fileall  $1024;
