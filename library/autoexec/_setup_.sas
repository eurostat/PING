...
* `G_PING_ROOTPATH` (and `G_PING_PROJECT`) as the path to your project repository _e.g._ you will have. 
  		- `G_PING_ROOTPATH=/ec/prod/server/sas/0eusilc/PING` if you run on the server, or
...
%macro _check_local_or_server_(_root_=);
			...
			* the one we use by default... the most common */
			...
			%if &sysscp = WIN %then %do;  /* local windows */
				...
				%do i = &start %to &finish;
					%let drv = %scan(&lst,&i);
					%if %sysevalf(%sysfunc(fileexist(&drv.:/PING/&file_to_check))) %then
						/* maybe on your drive...? */                        
						%let _path=&drv.:/PING;
					%else %if %sysevalf(%sysfunc(fileexist(&drv.:\home\PING\&file_to_check))) %then
						/* let us give it a second chance: maybe on your home directory...? */
						%let _path=&drv.:/home/PING;
			...
			%else %do; /* e.g., local linux/solaris server */
				%do i = &start %to &finish;
					...
					%if %sysevalf(%sysfunc(fileexist(/&drv./PING/&file_to_check))) %then
						/* maybe on your drive...? */                        
						%let _path=/&drv/PING;
					%else %if %sysevalf(%sysfunc(fileexist(/&drv./local/PING/&file_to_check))) %then 
						/* let us give it a second chance: maybe on your local directory...? */                        
						%let _path=/&drv./local/PING;
					%if &_path^= %then %goto quit;
				%end;
			%end;
 ...
 * 		%end;
 */
 %_check_local_or_server_(_root_=G_PING_ROOTPATH);
/*  However if you want to force to another directory, change the path below
 *		%let G_PING_ROOTPATH=; 
 */

%let &G_PING_PROJECT=&G_PING_ROOTDIR; /*&G_PING_ROOTPATH;*/
 /* You will then have:
...
%macro _default_setup_auto_; 
	...
	options SASAUTOS =(SASAUTOS 
						...
						"&G_PING_ROOTPATH/5.1_Integration/meta/"
						/*"&G_PING_ROOTPATH/5.3_Validation/pgm/"
						"&G_PING_ROOTPATH/5.3_Validation/meta/"*/
						"&G_PING_ROOTPATH/5.5_Extraction/pgm/"
						"&G_PING_ROOTPATH/5.5_Extraction/meta/"
						"&G_PING_ROOTPATH/5.5_Estimation/pgm/"
						"&G_PING_ROOTPATH/5.5_Estimation/meta/"
						"&G_PING_ROOTPATH/5.7_Aggregates/pgm/"
						"&G_PING_ROOTPATH/5.7_Aggregates/meta/"
						"&G_PING_ROOTPATH/6.3_Analysis/pgm/"
						/*"&G_PING_ROOTPATH/6.3_Analysis/meta/"*/
						"&G_PING_ROOTPATH/6.4_Anonymisation/pgm/"
						/*"&G_PING_ROOTPATH/6.4_Anonymisation/meta/"*/
						"&G_PING_ROOTPATH/7.1_Upload/pgm/"
						/*"&G_PING_ROOTPATH/7.1_Upload/meta/"*/
						"&G_PING_ROOTPATH/7.3_Dissemination/pgm/"
						/*"&G_PING_ROOTPATH/7.3_Dissemination/meta/"*/
						"&G_PING_ROOTPATH/7.4_Services/pgm/"
						/*"&G_PING_ROOTPATH/7.4_Services/meta/"*/
						"&G_PING_ROOTPATH/7.4_Visualisation/pgm/"
						/*"&G_PING_ROOTPATH/7.4_Visualisation/meta/"*/
						);
	options NOMRECALL;

%mend _default_setup_auto_;

%macro _default_setup_env_(legacy=yes, test=no, debug=no); /* default locations */
	...
	*%let LIBCFG=&G_PING_LIBCONFIG;
	...
	%let G_PING_MODULES=		&G_PING_ROOTPATH/5.5_Modules;
	...
	%let G_PING_SERVICES=		&G_PING_ROOTPATH/7.4_Services;
	...
	%if "&legacy"="YES" %then %do; 
		...
		%if "&test"="YES" %then %do;
			%let G_PING_IDBRDB=	&G_PING_ROOTDIR/IDB_RDB_TEST;
		%end;
		%else %do;
			%let G_PING_IDBRDB=	&G_PING_ROOTDIR/IDB_RDB;
		%end;

		%let db_raw=			&G_PING_ROOTDIR/main;
		%let db_valid=			&G_PING_ROOTDIR/main;
		%let db_extr=			&G_PING_IDBRDB;
		%let db_estim=			&G_PING_IDBRDB;
		%let db_upload=			&G_PING_IDBRDB/newcronos;

		%let G_PING_BDB=		&G_PING_ROOTDIR/BDB;
		%let G_PING_PDB=		&G_PING_ROOTDIR/pdb;
	...
	%let G_PING_DIRREPORT=		&G_PING_ROOTDIR/reports;
...
%macro _default_setup_par_;
	...
	%let G_PING_VARIABLExINDICATOR=		META_VARIABLExINDICATOR;   /* added by nobody on 07/12/2017 */
...
%macro _default_setup_;
	%*include "&G_PING_ROOTPATH/library/autoexec/_setup_.sas";
	%_default_setup_auto_;
	%_default_setup_env_(legacy=yes, test=no, debug=no); /* legacy environment and no test */
	%_default_setup_lib_;
	%_default_setup_var_;
	%_default_setup_par_;
...
%macro _test_setup_;
	%*include "&G_PING_ROOTPATH/library/autoexec/_setup_.sas";
	%_default_setup_auto_;
	%_default_setup_env_(legacy=yes, test=yes, debug=yes); /* legacy and test environment */
	%_default_setup_lib_;
	%_default_setup_var_;
	%_default_setup_par_;
%mend _test_setup_;
