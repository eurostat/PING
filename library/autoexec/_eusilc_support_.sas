...
options notes nosource source2 nomlogic nomprint nosymbolgen;
* options nonotes source source2 mlogic mprint symbolgen;
ODS HTML close;
...
%let G_PING_SUPPORT=&G_PING_ROOTDIR/7.5_Support;
...
%macro _project_autoname(_dir_=, _hier_=, _egp_=, _issue_=);
	%let _mac=&sysmacroname;
	%if %error_handle(ErrorInputParameter, 
			%macro_isblank(_dir_) EQ 1 or %macro_isblank(_hier_) EQ 1 or 
			%macro_isblank(_egp_) EQ 1 or %macro_isblank(_issue_) EQ 1,		
			txt=%bquote(!!! Output variables _DIR_, _HIER_, _EGP_ and _ISSUE_ need ALL to be set !!!), mac=&_mac) %then 
		%goto exit;

	%local _dir _hier _issue _egp _l;

	/* retrieve the directory */
	%let _pardir=&G_PING_ROOTDIR/%_egp_path(path=drive, parent=YES);
	%let _dir=&G_PING_ROOTDIR/%_egp_path(path=drive);

	/* check the structure */
	%if %dir_check(&_pardir/pgm) EQ 0 
			AND %dir_check(&_pardir/pgmX) EQ 0 
			AND %dir_check(&_pardir/data) EQ 0 %then %do;
		%let _dir=&_pardir;
		%let _hier=YES;
	%end;
	%else
...
/* 1) retrieve the project directory and its structure
*  2) retrieve the project name and the related issue, e.g. EUSILCPROD<XXX> where
* <XXX> is the request number */
%_project_autoname(_dir_=g_egpdir, _hier_=g_egphier, _egp_=g_egp, _issue_=g_issue);
...
%macro _prg_find(issue, _prglist_=);
	...
	%if "&g_egphier"="YES" %then 
		%let _dir=&g_egpdir/pgm;
	%else
		%let _dir=&g_egpdir;
