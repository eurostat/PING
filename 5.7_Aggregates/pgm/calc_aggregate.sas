...
%macro agg_compute(dsn
	...
	%let ctrydsn=_TMP&_mac;
	%ctry_select(&idsn, &geo, &time, &ctrydsn, ilib=&ilib, olib=WORK, 
				_pop_infl_=pop_infl, _run_agg_=run_agg, _pop_part_=pop_part, 
				take_all=, n_back=, sampsize=, max_sampsize=, thr_min=, thr_cum=, 
				cds_popxctry=&cds_popxctry, cds_ctryxzone=&cds_ctryxzone, clib=&clib);
	...
	/* set the rule on currency/unit flag when it exists */
 	...
 	%if &Utab=DI10 or &Utab=DI02 %then 
		%let where=%quote(currency='EUR');	
	%else %if &Utab=DI03 or &Utab=DI04 or &Utab=DI05 or &Utab=DI07 or &Utab=DI08 or &Utab=DI09 
			or &Utab=DI13 or &Utab=DI13B or &Utab=DI14 or &Utab=DI14B %then
		%let where=%quote(unit='EUR');	
	%else
		%let where=;

	%local MACHINE_EPSILON;
	%if %symexist(G_PING_MACHINE_EPSILON) %then %let MACHINE_EPSILON=&G_PING_EPSILON;
	%else  										%let MACHINE_EPSILON=%sysevalf(1./10**14);

	%local flag;
	%if %sysevalf(&pop_infl - 1. >= &MACHINE_EPSILON) OR ("&zone"="EU27" AND &year<2007) %then %do; 
		%let flag='s';
	%end;

	%aggregate_flag(&zone, &ctry_glob, &year, &ctry_tab, _flag_=flag_zone);

	%ctry_population_join(&idsn, &ctrydsn, &time, &odsn, ilib=, olib=, where=&where, 
						cds_popxctry=&cds_popxctry, clib=&clib);
