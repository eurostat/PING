...
%macro aggregate_build(/*input*/    dsn, ctry_glob, year, 
	...
	%if %macro_isblank(_pop_part_) %then 	%let _pop_part_=pop_part;

	%if &G_PING_DEBUG %then 
		%put input parameters: ny=&nyear_back, s=&sampsize, th_reach=&thres_reach, th_presence=&thres_presence;
	...
	%if %macro_isblank(pop_file) %then %do;
		%if %symexist(G_PING_POPULATIONxCOUNTRY) %then 	%let pop_file=&G_PING_POPULATIONxCOUNTRY;
		%else											%let pop_file=POPULATIONxCOUNTRY;
	%end; 
	...
	%let max_ctry_n=%list_length(&ctry_glob);
	...
	%ctry_population(&ctry_glob, &year, _pop_size_=pop_glob);
	%if %error_handle(ErrorInputParameter, 
			&pop_glob EQ ,		
	...
	* we use instead the force_overwrite variable below */
	...
	%do %while (1);
		...
		%do %while (1);
			...
			%end;
			%ctry_select(&dsn, &ctry_miss, &year_available, &ctry_tab, ilib=&lib, sampsize=&s_size);
			 ...
			 * to (year) */
			%var_to_list(&ctry_tab, geo, _varlst_=ctry_part, lib=WORK);
		     ...
		     * estimation!) */
			%ctry_population(&ctry_part, &year, _pop_size_=&_pop_part_);
			%if &G_PING_DEBUG %then %put population of added countries: &&&_pop_part_;
	%put ctry_part=&ctry_part;
	%put &_pop_part_=&&&_pop_part_;
			...
			%population_compare(&pop_glob, &&&_pop_part_, _pop_infl_=&_pop_infl_, _ans_=&_run_agg_, 
								pop_thres=&thres);
			...
			%put ctry_miss=&ctry_miss;
			%put ctry_part=&ctry_part;
			%let ctry_miss=%list_difference(&ctry_miss, &ctry_part, sep=%str(%"));
...
%macro _example_aggregate_build;
	...
	%ds_print(&tab_part, title="countries available in 2016 - Tpres=&thres_presence");
	...
	%aggregate_build(&dsn, &ctry_glob, &year, &tab_part, 
					 ...
					 pop_file=&pop_file, lib=rdb);
	%put 	result in: pop_infl=&pop_infl, pop_part=&pop_part, run_agg=&run_agg;
	%ds_print(&tab_part, title="countries available in 2016 and 2015 - Tpres=&thres_presence, Treach=&thres_reach");
	...
	%put 	result in: pop_infl=&pop_infl, pop_part=&pop_part, run_agg=&run_agg;
	%ds_print(&tab_part, title="countries available in 2016 and 2015 - Take all");
