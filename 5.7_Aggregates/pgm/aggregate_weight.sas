...
%macro aggregate_weight(/*input*/ zone, grpdim, tab, pop_part, pop_infl,  
	...
	%local unitOK;
	%let unitOK=%var_check(&tab, unit, lib=WORK);
	...
	PROC SQL;
		...
		%if &unitOK EQ 0 %then %do;
		/* %if &dsn =LI43 or &dsn =lvhl23 or &dsn =mddd23 or &dsn=peps13 %then %do; */
			(case when %upcase(unit) in ("THS_PER", "THS_CD08") then ivalue
				else (ivalue * totwgh) 
				end) AS wivalue ,
			(SUM(CALCULATED wivalue)) AS SUM_OF_wivalue,
			(case when %upcase(unit) in ("THS_PER", "THS_CD08") then (CALCULATED SUM_of_wivalue * &pop_infl)
