/** 
## META_POPULATIONxCOUNTRY {#meta_populationxcountry}
...
%macro meta_populationxcountry(cds_popxctry=, cfg=, clib=);
	...
	%if %macro_isblank(cds_popxctry) %then %do;
		...
		%else											%let cds_popxctry=META_POPULATIONxCOUNTRY;
	%end;
	...
	%if %macro_isblank(cfg) %then %do;
		%if %symexist(G_PING_AGGREGATES) %then 		%let cfg=&G_PING_AGGREGATES/meta;
		%else										%let cfg=&G_PING_ROOTPATH/5.7_Aggregates/meta;
	%end;
...
%mend meta_populationxcountry;


%macro _example_meta_populationxcountry;
	...
	%put;
%mend _example_meta_populationxcountry;

/* Uncomment for quick testing
options NOSOURCE MRECALL MLOGIC MPRINT NOTES;
%_example_meta_populationxcountry;
*/
